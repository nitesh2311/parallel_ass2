...
int removeP(int id,int digit,val array)
{
  ...
  for(i=0;i<len;i++)
  { 
    if(array[arrstr[id].peer[i]].set==-1)
    {if(array[arrstr[id].peer[i]].value[digit]!=-1)
    { array[arrstr[id].peer[i]].value[digit] = -1;
      array[arrstr[id].peer[i]].len--;
      if(array[arrstr[id].peer[i]].len==0 && array[arrstr[id].peer[i]].set== -1)
      {
        return 2;
      }
      if(array[arrstr[id].peer[i]].set==digit)
      {
        return 2;
      }
    }
    ...
    if(array[idn].len==1 && array[idn].set == -1)   {
      ...
      { 
        ...
        { f++;
          ...
          int ret = eliminate2(idn,j,array);
          if(ret == 2)return ret;
...
{ int dig[N];
  ...
  for(i=0;i<N;i++)
  {
    if(leng[i]==1)
    {
      ...
      int ret = eliminate(dig[i],i,array);
      if(ret == 2)return ret;
    }
  }
...
{   
  ...
  { 
    ...
    for(j=0;j<N;j++)
    {
      if(len[j]==1)
      { arr[dig[j]].set=j;
        int ret= eliminate2(dig[j],j,arr);
        if(ret==2)return ret;
      }
      else if(len[j]==0)
      {
...
{
  int ret = removeP(id,dig,arr);
  if(ret == 2)return ret;
  ret = setP(id,arr);
  if(ret == 2)return ret;
 // ret = lonerangeP(id,arr);
  return 0;
}
...
{ int i,j;
  ...
  for(i=0;i<N*N;i++)
  {
    ...
    if(src[i].set==dest[i].set && src[i].set!=-1)
    {

    }
    else if(src[i].set==-1 && dest[i].set==src[i].set && dest[i].len == src[i].len)
    {
...
int search(val array,int** out)
{
  ...
  for(i=0;i<N;i++)
  {
    ...
    if( array[id].value[i]!=-1)
    {
      ...
      newval[id].set=i;
      int ret = eliminate(id,i,newval);
      if(ret==2){

      }
      else{
      ans = search(newval,out);
      if(ans == 1)return ans;
      }
...
int init(val arr,int** out)
{
  ...
  for(i=0;i<N;i++)
  {
    for(j=0;j<N;j++)
    {
      ...
      if(out[i][j]!=0)
      {
        ...
        int ret = eliminate(id,out[i][j]-1,arr);
        if(ret == 2)
        {
            return -1;
        }
  ...
  int ret = lonerange(arr);
  if(ret==2){

            printf("wrong in init2 :\n");
            return -1;
  }
